...
(function () {
  ...
  function openAddStaffModal() {
    ...
    document.getElementById("addClinicStaffModal")?.classList.add("show");
  }

  function closeAddStaffModal() {
    document.getElementById("addClinicStaffModal")?.classList.remove("show");
  }
  ...
  async function createClinicStaff(event) {
    ...
    try {
      ...
      const createStaff = functions.httpsCallable("createClinicStaff");
      await createStaff({ displayName, email, password, role, department });

      closeAddStaffModal();
      document.getElementById("addClinicStaffForm")?.reset();
      Core.showToast(`${displayName} 已建立`);
      await window.InneraClinicStaff.load();
    } catch (error) {
      console.error("[Innera] 新增院所成員失敗", error);
      let message = "建立院所成員失敗";
      if (error.code === "functions/already-exists") message = "這個 Email 已經有帳號";
      if (error.code === "functions/permission-denied") message = "你沒有新增院所成員的權限";
  ...
  async function disableClinicStaff(staffUid, staffName) {
    ...
    if (!window.confirm(`確定要停用 ${staffName} 嗎？`)) return;
    ...
    try {
      ...
      console.error("[Innera] 停用院所成員失敗", error);
      let message = "停用院所成員失敗";
      if (error.code === "functions/permission-denied") message = "你沒有停用院所成員的權限";
      if (error.code === "functions/not-found") message = "找不到這位院所成員";
      Core.showToast(message);
    }
  }
  ...
  async function enableClinicStaff(staffUid, staffName) {
    ...
    if (!window.confirm(`確定要重新啟用 ${staffName} 嗎？`)) return;
    ...
    try {
      ...
      console.error("[Innera] 啟用院所成員失敗", error);
      let message = "啟用院所成員失敗";
      if (error.code === "functions/permission-denied") message = "你沒有啟用院所成員的權限";
      if (error.code === "functions/not-found") message = "找不到這位院所成員";
      Core.showToast(message);
    }
  }
